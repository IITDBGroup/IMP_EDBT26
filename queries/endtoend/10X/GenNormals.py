    ...
    for line in lines:
        ...
        if line[0] == 'Q':
            ...
            if uNum[0] == '-':
                ...
                qfile.write(f'delete from edb1 where a >= {uNum[1:]} and a < {(int) (uNum[1:]) + 10};')
                qfile.close()

            else :
                qfile = open(f'./{dir}/nor_{qidx}.sql', 'w')
                # qfile.write(f'insert into edb1 select * from edb1_backup where a = {uNum[1:]};')
                qfile.write(f'insert into edb1 select * from edb1_backup where a >= {uNum[1:]} and a < {(int) (uNum[1:]) + 10};')
                qfile.close()
