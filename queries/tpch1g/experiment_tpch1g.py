...
def recQ(q, refreshTable):
    ...
    for n in range(1, num+1, 1):
        ...
        tupBackSQL = f'insert into {oriT} select {att} from {updT} where updident = -1'
...
#        # process_one_query(q)
#    for q in options.queries:
#        logfat(f"process query q{q} rec")
#        recQ(q, refreshTable)
    for q in options.queries:
        logfat(f"process query q{q} inc")
        incQ(q, refreshTable)
...
if __name__ == '__main__':
    ...
    # db setting
    ap.add_argument('-d', '--db', type=str, default='dbtpch1g',
                    help='database name')
