...
# GPROM_BIN = '/home/oracle/pengyuan/gprom_f/gprom/src/command_line/gprom'
...
def capReuseRun(q, capIndFilePath):
    ...
    for line in capIndLines:
        ...
        if QorU[0] == 'U':
            ...
            with open(f'./psFiles/ps_{q}_{bucketRanges}', 'w') as ff:
...
def reuseRun(q, indLine, bucketRanges):
    ...
    QorU = lines[0].strip().split('_')
    bucketRanges = lines[2].strip()
    psFile = f'./psFiles/ps_{q}_{bucketRanges}'
...
def incrementalRun(q, incIndFilePath):
    ...
    for line in incLines:
        ...
        if lineCells[0][0] == 'U':
            ...
            ## inc upd ps
            insOrDel = lineCells[1].strip()
            updateAValue = lineCells[1].strip()[1:]
            isInsert = True
            # delta table name
            deltaTblName = ''
            if insOrDel[0] == '-':
                deltaTblName = f'edb1_del_a_{updateAValue}'
            else:
                deltaTblName = f'edb1_ins_a_{updateAValue}'

            updPSList = lineCells[2].strip()
            if updPSList =='[NN]':
                ffff=open('incinforlog', 'a')
                ffff.write(f'q-{q}:Q-{QorU[1]}:INC:NN\n')
                ffff.close()
            else:
                listContents = updPSList[1:-1]
                ffff=open('incinforlog', 'a')
                ffff.write(f'q-{q}:Q-{QorU[1]}:INC:{listContents}\n')
                ffff.close()
                lists = listContents.strip().split(',')
                # delta table
                for bRange in lists:
                    storeName = f'{PREFIX}{q}_{bRange}'
                    isDataStoreBack = 'FALSE'

                    incInfile = f'./incFiles/inc_{bRange}.sql'
                    incOutfile = f'{options.resultdir}/runtime_incre_{q}_U{QorU[1]}_{bRange}.csv'

                    gprom_time(incInfile, incOutfile, 10, 'TRUE', f"{options.tables[q][0]}", f"{deltaTblName}", f'{selectionPushDown}', f'{isJoinCopy}', f'{isJoinBF}', 'updident', f'{storeName}', f'{isDataStoreBack}')

                    incOutfile = f'{options.resultdir}/runtime_dummy.csv'
                    gprom_time(incInfile, incOutfile, 1, 'TRUE', f"{options.tables[q][0]}", f"{deltaTblName}", f'{selectionPushDown}', f'{isJoinCopy}', f'{isJoinBF}', 'updident', f'{storeName}', 'TRUE')
