...
refreshTable = False
ATTRs = {
    'crimes' : ' id, case_number, date, block, iucr, primary_type, description, location_description,arrest, domestic, beat, district, ward, community_area, fbi_code, x_coordinate, y_coordinate,year, updated_on, latitude, longitude, location '
}
...
def recQ(q, refreshTable):
    ...
    for n in range(1, num+1, 1):
        ...
        updateSQL = f'delete from {oriT} where exists(select * from {updT} where {oriT}.id = {updT}.id and {updT}.updident = -1)'

        updateSQL1 = f'delete from {oriT} where exists(select * from {updT} where {oriT}.id = {updT}.id and {updT}.updident = 1)'
        att = ATTRs[oriT]
...
if __name__ == '__main__':
    ...
    # db setting
    ap.add_argument('-d', '--db', type=str, default='dbcrimes',
                    help='database name')
