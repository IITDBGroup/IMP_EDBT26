...
def main(args):
    ...
    qDir = getQueryDir()
    print(qDir)
    if not os.path.exists(f'{qDir}/batchF'):
        os.mkdir(f'{qDir}/batchF')
    # for ind in inds:
    if True:
        psql_run(f'drop table if exists dy{PREFIX}_delta;')
        psql_run(f'create table dy{PREFIX}_delta as select * from t1gbjoin where id = 1;')
        psql_run(f'truncate dy{PREFIX}_delta;')
        psql_run(f'alter table dy{PREFIX}_delta add column if not exists updident int;')
        psql_run(f'alter table dy{PREFIX}_delta add column if not exists updateversion int;')

        indC = open(f'{qDir}/seq_indicator')
        indLines = indC.readlines()
        indC.close()
        if (len(indLines) != totalQNum):
            print("ERROR LOAD")
            return
        for line in indLines:
            lineCells = line.split(':')

            batchF = open(f'{qDir}/batchF/upd_{PREFIX}_{lineCells[0].strip()}.sql', 'w')
            randVal = lineCells[1].strip()
            batchF.write(f'insert into dy{PREFIX}_delta select *, -1, {lineCells[0]} from t1gbjoin where id >= {randVal} and id <= {randVal} + 149;\n')
            batchF.close()
            infiel = f'{qDir}/batchF/upd_{PREFIX}_{lineCells[0].strip()}.sql'
            outfile = f'{options.resultdir}/updBatch_{lineCells[0]}.txt'
            psql_time(infiel, outfile, 1)

        psql_run(f'create index idx_{PREFIX}_delta on dy{PREFIX}_delta (id)')
        psql_run(f'create index idx_{PREFIX}_updver on dy{PREFIX}_delta (updateversion)')
