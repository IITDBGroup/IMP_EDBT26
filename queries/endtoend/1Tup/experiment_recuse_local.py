...
IS_TEST = True
GPROM_BIN = '/Users/pengyuanli/GPROMS_SEP/gprom/src/command_line/gprom'
PSQL_BIN = '/Users/pengyuanli/trash/post16/postgres16_install/bin/psql'
################################################################################
# utils# log
def log(msg):
    print(msg)
    ...
    for line in capIndLines:
        ...
        if QorU[0] == 'U':
            ...
            if IS_TEST == True:
                ...
                ff = open(f'seq_ru_{q}', 'a')
                ff.write(f'Q: {QorU[1]}, cap: {bucketRanges}\n')
                ff.close()
                # continue
            else:
...
def reuseRun(q, indLine, bucketRanges):
    ...
    if IS_TEST == True:
        psql_time(infile, outfile, 1)
        ff = open(f'seq_ru_{q}', 'a')
        ff.write(f', reuse where cond len:{(psClauster)}')
    else:
        psql_time(infile, outfile, 10)
...
if __name__ == '__main__':
    ...
    # db setting
    ap.add_argument('-d', '--db', type=str, default='test',
                    help='database name')
    ap.add_argument("-H", "--host", type=str, default="127.0.0.1",
                    help="database host")
    ap.add_argument("-u", "--user", type=str, default="pengyuanli",
                    help="database name")
    ap.add_argument("-P", "--password", type=str, default="test",
                    help="database password")
    ap.add_argument("-p", "--port", type=int, default=5454,
                    help="database port")
