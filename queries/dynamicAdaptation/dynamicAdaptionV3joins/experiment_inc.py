...
def testOption(infile, outfile, repetitions, direcDelta, updateTable, deltaTable, selectionPushDown, isJoinCopy, isJoinBF, updIdent, queryName, isDataStoreBack, isBatch, batchStart, batchEnd):
    gprom_options = ['-backend', 'postgres', '-host', f'{options.host}', '-user', f'{options.user}', '-passwd', f'{options.password}', '-port', f'{str(options.port)}', '-db', f'{options.db}', '-loglevel', '0', '-Pexecutor','sql', '-ps_binary_search', 'TRUE', '-update_ps_direct_delta', 'TRUE', '-update_ps_updated_table', str(updateTable), '-update_ps_delta_table', str(deltaTable), '-timing', '-update_ps_selection_push_down', f'{selectionPushDown}', '-update_ps_copy_delta_join', 'TRUE', '-update_ps_join_using_bf', f'{isJoinBF}', '-update_ps_delta_table_updident', f'{updIdent}', '-update_ps_query_name', f'{queryName}', '-update_ps_store_new_state', f'{isDataStoreBack}', '-update_ps_batching_update', 'TRUE', '-update_ps_begin_update_version', f'{str(batchStart)}', '-update_ps_end_update_version', f'{str(batchEnd)}']
    print(gprom_options)

PREFIX = 'v3'
def dynamic_inc():
    eachs = [1, 5, 10, 50, 100, 500, 1000]
    qDir = f'{os.getcwd()}'
    for each in eachs:
        start = 1
        end = start + each - 1
        while start < 1000:
            updateTable = 't1gbjoin'
            deltaTblName = f'dy{PREFIX}_delta'
            selectionPushDown = 'TRUE'
            isJoinBF = 'TRUE'
            isJoinCopy = 'TRUE'
            storeName = f'dyEach{each}'
            # updVersion = lineCells[0].strip()
            batchStart = f'{str(start)}'
            batchEnd = f'{str(end)}'
            incInfile = f'{qDir}/inc.sql'


            incOutfile = f'{options.resultdir}/lEach{each}_{str(batchStart)}_{str(batchEnd)}'

            gprom_time_batching(incInfile, incOutfile, 10, 'TRUE', f"{updateTable}", f"{deltaTblName}", f'{selectionPushDown}', f'{isJoinCopy}', f'{isJoinBF}', 'updident', f'{storeName}', 'FALSE', 'TRUE', batchStart, batchEnd)

            incOutfile = f'{options.resultdir}/lEach{each}_{str(batchStart)}_{str(batchEnd)}_bq'
            gprom_time_batching(incInfile, incOutfile, 1, 'TRUE', f"{updateTable}", f"{deltaTblName}", f'{selectionPushDown}', f'{isJoinCopy}', f'{isJoinBF}', 'updident', f'{storeName}', 'TRUE', 'TRUE', batchStart, batchEnd)

            # testOption(incInfile, incOutfile, 1, 'TRUE', f"{updateTable}", f"{deltaTblName}", f'{selectionPushDown}', f'{isJoinCopy}', f'{isJoinBF}', 'updident', f'{storeName}', 'TRUE', 'TRUE', batchStart, batchEnd)
            start += each
            end += each
