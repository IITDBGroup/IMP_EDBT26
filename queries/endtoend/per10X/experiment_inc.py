    ...
    for line in incLines:
        ...
        if lineCells[0][0] == 'U':
            ...
            if updWhich[0] == '-':
                ...
                bfOpen.write(f'insert into {PREFIX}_delta_{q} select *, -1, {updNumVal} from edb1 where id >= {updWhich[1:]} and id < {(int)(updWhich[1:]) + 20};\n')
                appendToFile(auxFile, f'     insert into {PREFIX}_delta_{q} select *, -1, {updNumVal} from edb1 where id >= {updWhich[1:]} and id < {(int)(updWhich[1:]) + 20};\n')
            else:
                bfOpen.write(f'insert into {PREFIX}_delta_{q} select *, 1, {updNumVal} from edb1_backup where id >= {updWhich[1:]} and id < {(int)(updWhich[1:]) + 20};\n')
                # bfOpen.write(f'insert into {PREFIX}_delta_{q} select *, 1, {updNumVal} from edb1 where a = {updWhich[1:]};\n')
                # appendToFile(auxFile, f'     insert into {PREFIX}_delta_{q} select *, 1, {updNumVal} from edb1_backup where a = {updWhich[1:]};\n')
                appendToFile(auxFile, f'     insert into {PREFIX}_delta_{q} select *, 1, {updNumVal} from edb1_backup where id >= {updWhich[1:]} and id < {(int)(updWhich[1:]) + 20};\n')
            bfOpen.close()
